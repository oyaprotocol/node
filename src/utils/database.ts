	...
	for (let attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
		try {
			...
			if (lastError.message.includes('does not support SSL')) {
				...
				logger.warn(
					'‼️ Database does not exist. Create it with: bun run db:create'
				)
...
export async function validateSchema(
	...
	try {
		...
		if (missingTables.length > 0) {
			...
			logger.warn(
				'‼️ Database tables are missing. Create them with: bun run db:setup'
			)
			...
			return {
				success: false,
				error: `Missing required tables: ${missingTables.join(', ')}. Run 'bun run db:setup' to create them`,
