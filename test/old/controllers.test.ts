...
describe('API Tests', () => {
	describe('POST /bundle', () => {
		...
		it('should return 400 for invalid data', async () => {
			const res = await request(app).post('/bundle').send({
				bundle: null,
				nonce: 'invalid',
			})
	...
	describe('GET /bundle/:nonce', () => {
		it('should get a bundle by nonce', async () => {
			await pool.query('INSERT INTO bundles (bundle, nonce) VALUES ($1, $2)', [
				JSON.stringify({ some: 'data' }),
				1,
			])

			const res = await request(app).get('/bundle/1').send()
		...
		it('should return 404 for non-existing bundle', async () => {
			const res = await request(app).get('/bundle/999').send()
	...
	describe('GET /bundle', () => {
		it('should get all bundles', async () => {
			const res = await request(app).get('/bundle').send()
			expect(res.status).to.equal(200)
			expect(res.body).to.be.an('array')
		})
	})
	...
	describe('POST /cid', () => {
		it('should save a CID', async () => {
			const res = await request(app).post('/cid').send({
				cid: 'somecid',
				nonce: 1,
			})
			expect(res.status).to.equal(201)
	...
	describe('GET /cid/:nonce', () => {
		it('should get CIDs by nonce', async () => {
			await pool.query('INSERT INTO cids (cid, nonce) VALUES ($1, $2)', [
				'somecid',
				1,
			])

			const res = await request(app).get('/cid/1').send()
			expect(res.status).to.equal(200)
			expect(res.body).to.be.an('array')
		...
		it('should return 404 for non-existing CIDs', async () => {
			const res = await request(app).get('/cid/999').send()
			expect(res.status).to.equal(404)
		})
	})
	...
	describe('GET /balance/:vault', () => {
		it('should get balances for an vault', async () => {
			...
			const res = await request(app).get('/balance/vault1').send()
			expect(res.status).to.equal(200)
			expect(res.body).to.be.an('array')
	...
	describe('GET /balance/:vault/:token', () => {
		it('should get balance for an vault and token', async () => {
			...
			const res = await request(app).get('/balance/vault1/token1').send()
			expect(res.status).to.equal(200)
			expect(res.body).to.be.an('array')
			expect(res.body[0]).to.have.property('vault', 'vault1')
			expect(res.body[0]).to.have.property('token', 'token1')
	...
	describe('POST /balance', () => {
		it('should update balance for an vault and token', async () => {
			const res = await request(app).post('/balance').send({
				vault: 'vault1',
				token: 'token1',
				balance: 200,
			})
	...
	describe('GET /nonce/:vault', () => {
		it('should get nonce for an vault', async () => {
			await pool.query('INSERT INTO nonces (vault, nonce) VALUES ($1, $2)', [
				'vault1',
				1,
			])

			const res = await request(app).get('/nonce/vault1').send()
			expect(res.status).to.equal(200)
			expect(res.body).to.have.property('nonce', 1)
		})

		it('should return 404 for non-existing nonce', async () => {
			const res = await request(app).get('/nonce/nonexistentvault').send()
			expect(res.status).to.equal(404)
		})
	})
